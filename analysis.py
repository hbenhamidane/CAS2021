...
'''
When keeping all targets leads to a lot of NAs:
    ...
    - One alternative is replacing Na with 0, then filter targets using various
    feature selection methods (see https://scikit-learn.org/stable/modules/feature_selection.html)
'''
# Option A: Drop Na
# # before dropping NAs, one should check consequence on representativity
# # e.g. there is no data for 'Dissolved organic carbon (DOC)' for groundwater
# dfa_g = dfa_g.dropna()
# Option B: impute and select features
dfa_g.fillna(value=0, inplace=True)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!
TO DO:
    - check if scaling should/could be done before filling Na
    - check with filtering features
        + criteria of std might be too random if done before scaling
'''
